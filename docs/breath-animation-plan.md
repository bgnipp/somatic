...
## Physician follow-up #2 (in-flight messages, 2026-08-15 11:28 AM)

Messages + three Essential Anatomy screenshots preserved at `context/transcripts/2026-08-15-physician-inflight-muscle-roster.txt` and `context/muscle-roster/`. Assessment:

1. **Pec pair choreography (implementable in the front view now).** From the "BT osteopathy animation" she cites: **pec minor assists inhalation (contracts); pec major assists by relaxing.** The pec paths already exist in `SuperficialLayer` (`anatomy-pec`); this is a new `pecs_minor`/`pecs_major` structure split (or a single `pecs` structure with the major's relax curve, minor deferred to Track H — the minor is deep to the major and barely distinguishable at schematic fidelity). We do **not** have a link to the BT osteopathy video itself — get it from her before encoding details beyond the pec pair.
2. **"The animation needs to include every muscle listed" — the roster is mostly posterior.** Her screenshots select: iliocostalis cervicis, longissimus cervicis, intertransversarii thoracis & cervicis posteriores, levatores costarum longi/brevis, subclavius. All but subclavius are **deep posterior spinal muscles — invisible in a front view** (Essential Anatomy itself shows them from behind). This converges with the rib-geometry video's posterior-expansion theme: the product now has two independent physician-endorsed pushes toward a **back view**. The honest options are (a) a back-view figure (already a parked mvp-plan question, now upgraded), or (b) Track-H-era 3D-ish layering. Either is beyond schematic front-view placeholders — needs a scoping decision with her, not silent partial inclusion.
3. **"We can input the action of each muscle … this is the minutia required."** She is describing a **muscle catalog**: a data table of muscles with origin/insertion/clinical action, from which choreography is derived. Our `GuideStructureId` + `ActivationFn` architecture is exactly this shape — but at ~10 structures, hand-coded. Scaling to "every muscle listed" means the catalog becomes data (JSON), not code, with per-muscle action metadata. That's an architecture step, not a patch.
4. **The composer — a genuinely new feature.** "We will select a group of muscles to engage and activate, they will light up in the order selected (same with group of relaxing muscles)." That is an **authoring mode**: pick an engage group and a relax group, ordered, and the app generates the animation — user-composed sequences instead of (or alongside) fixed scripts. Architecturally friendly: a composed selection compiles to a `GuideScript` (ordered activation onsets = phase offsets), so the renderer doesn't change. But it needs its own mini-plan (selection UI, ordering UI, persistence, and how it coexists with the three canonical scripts). **Do not improvise this in a patch.**

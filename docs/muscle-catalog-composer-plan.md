...
2. **Sequence lists:** three ordered chip rows ("Engage — in order" / "Release — in order" / "Stabilize — in order") with per-chip remove (✕) and drag-free reorder via ◂ ▸ buttons (no drag-and-drop dependency). Stabilize (added 2026-08-16, D3) ramps to partial tension (~0.45) with the same staggered-onset timing — faint red, her balancing/opposing stabilizer role.

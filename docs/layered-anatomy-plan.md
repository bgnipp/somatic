...
*(Physician feedback addendum, 2026-08-15 — see `docs/breath-animation-plan.md`, now the implementation spec for the "Guide" view. Layer-roster consequences decided there: the pelvic floor (levator ani dome) joins the `deep` layer — relabeled "Diaphragm & core" — rather than becoming a seventh depth stop; schematic upper-trap outlines join `superficial`; scalene hints render on the neck in Guide view only; platysma and any figure-crop extension are deferred pending her answer. She also wants the whole figure to visibly oscillate with breath (ribs widening, silhouette swelling), which builds on the G4 diaphragm morph and is specified in that doc's B3 phase. Track H renderers: `deep.png` should include the pelvic floor dome; otherwise the render list is unchanged.)*
